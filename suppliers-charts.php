...
<style type="text/css">
  ...
  <div class="dropdown-menu shadow-sm" aria-labelledby="dropdownMenuButton">
    <a class="dropdown-item" href="?time=<?php echo($period);?>&status=Batches-Supplied&chart=<?php echo($chart); ?>">By Batches Supplied</a>
    <a class="dropdown-item" href="?time=<?php echo($period);?>&status=expenditure&chart=<?php echo($chart); ?>">By Expenditure</a>
  ...
  <div class="dropdown-menu shadow-sm" aria-labelledby="dropdownMenuButton">
    ...
    <a class="dropdown-item" href="?time=1&status=<?php echo($status);?>&chart=<?php echo($chart); ?>">Last 30 Days</a>
     ...
     Suppliers By <?php echo($status);?> for last <?php echo($period);?> Months
   </small></center><br>
            </div>
...
$ct_active = mysqli_query($con,"SELECT * FROM suppliers WHERE supplier_status='active' && created_at >= (DATE_SUB(CURDATE(), INTERVAL $period MONTH)) ");
$ct_actve = mysqli_num_rows($ct_active);

$ct_suspended = mysqli_query($con,"SELECT * FROM suppliers WHERE supplier_status='suspended' && created_at >= (DATE_SUB(CURDATE(), INTERVAL $period MONTH)) ");
...
chart.render();
  ...
  <?php
} elseif ($status=='Batches-Supplied') {
...
$request = $domain."core/json-suppliers-consumption.php?suppliers_by_consumption";
...
<?php }
 ...
 elseif ($status=='expenditure') {
  

$request = $domain."core/json-suppliers-expenditure.php?suppliers_by_expenditure";
