        ...
        <div   class="col-lg-12  mb-lg-0 mb-4">
          <div class="card z-index-2 h-100">
            <div class="card-header pb-0 pt-3 bg-transparent">
              <h6 class="text-capitalize">Manage Sales <button data-toggle="modal" data-target="#newSales" style="float:right;" class="btn bg-gradient-primary"> <i class="fa fa-plus-circle"></i> </button> </h6>
...
<?php
$gt_sale =mysqli_query($con,"SELECT * FROM sales ORDER BY id DESC");
...
<select required class="form-control rounded-pill" name="customer">
...
while ($ro=mysqli_fetch_array($gt_customer)) {
  // code...?>
<option value="<?php echo(htmlentities($ro['id']));?>"><?php echo(htmlentities($ro['customername']));?></option>
  <?php
}
?>
  </select>
<br>
<input required type="text" readonly class="form-control rounded-pill" name="receiptnumber" placeholder="Receipt Number" value="<?php echo((rand(1000,100000000)));?>">
<br>
<select required class="form-control rounded-pill" name="productname">
  <?php
...
<option value="<?php echo(htmlentities($pro['id']));?>"><?php echo(htmlentities($pro['itemsupply']));?></option>
  ...
  <small class="text-center">Select Handler</small>
       ...
       <select required class="form-control rounded-pill" name="handler">
<?php
...
<?php } ?>
  ...
  <br>
  <button name="add_sale" class="btn bg-gradient-danger" style="width:68%;"> Add Sale </button>
</form>
