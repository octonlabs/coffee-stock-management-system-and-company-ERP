...
<style type="text/css">
  ...
  <div class="dropdown-menu shadow-sm" aria-labelledby="dropdownMenuButton">
    <a class="dropdown-item" href="?time=<?php echo($period);?>&status=Purchases&chart=<?php echo($chart); ?>">By Purchases</a>
  ...
  <div class="dropdown-menu shadow-sm" aria-labelledby="dropdownMenuButton">
    ...
    <a class="dropdown-item" href="?time=1&status=<?php echo($status);?>&chart=<?php echo($chart); ?>">Last 30 Days</a>
     ...
     Customers By <?php echo($status);?> for last <?php echo($period);?> Months
   </small></center><br>
            </div>
...
$ct_active = mysqli_query($con,"SELECT * FROM customers WHERE customer_status='active' && created_at >= (DATE_SUB(CURDATE(), INTERVAL $period MONTH)) ");
$ct_actve = mysqli_num_rows($ct_active);

$ct_suspended = mysqli_query($con,"SELECT * FROM customers WHERE customer_status='suspended' && created_at >= (DATE_SUB(CURDATE(), INTERVAL $period MONTH)) ");
...
$request = $domain."core/json-customers-consumption.php?customers_by_consumption";
