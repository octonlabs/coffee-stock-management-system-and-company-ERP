...
<style type="text/css">
  ...
  <div class="dropdown-menu shadow-sm" aria-labelledby="dropdownMenuButton">
    ...
    <a class="dropdown-item" href="?time=1&status=<?php echo($status);?>&chart=<?php echo($chart); ?>">Last 30 Days</a>
     ...
     Supplies By <?php echo($status);?> for last <?php echo($period);?> Months
   </small></center><br>
            </div>
...
$ct_active = mysqli_query($con,"SELECT * FROM supplies WHERE status='active' && created_at >= (DATE_SUB(CURDATE(), INTERVAL $period MONTH)) ");
$ct_actve = mysqli_num_rows($ct_active);

$ct_suspended = mysqli_query($con,"SELECT * FROM supplies WHERE status='suspended' && created_at >= (DATE_SUB(CURDATE(), INTERVAL $period MONTH)) ");
...
$request = $domain."core/json-supplies-consumption.php?supplies_by_consumption";
...
$request = $domain."core/json-supplies-expenditure.php?supplies_by_expenditure";
